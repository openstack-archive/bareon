...
Sometimes after a node is deleted in Nova, it can remain in a state where it cannot be re-scheduled. To get around this you can edit the database and reset it back to a 'deleted' state.
...
Check that the appropriate kernel modules are loaded for virtualization.
