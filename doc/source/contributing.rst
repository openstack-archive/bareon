...
Fuel Python Agent functional testing performed against standard kernel/ramdisk
...
(fpa-func-test). The framework itself uses libvirt (python bindings) to
configure network, spawn a Slave VM running ramdisk. In future we may add
support for baremetal slaves. Framework resides in a standalone repo on stash.
Functional tests, as well as commonly changed parts of test data (node
templates, deploy_configs, etc) are located in fuel-agent tree, so that each
pull request introducing the new functionality to fuel-agent can also carry
...
- Install fpa-func-test from source:
...
- Configure fpa-func-test framework
...
For different use cases you can define a custom environment. To run tests
using own environment:

.. code-block:: console

    $ export BUILD_ENV=/path/to/fuel_agent_pr_env.sh
    $ cd ~/bareon
    $ tox
