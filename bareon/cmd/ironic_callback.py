...
class App(object):
    def __init__(self):
        ...
        self.task_mapping = _TaskMapping()
    ...
    def _make_step(self, data):
        try:
            task_cls = self.task_mapping.name_to_task[data.action]
        except KeyError:
            raise InadequateRequirementError(
                'There is no task "{}"'.format(data.action))
        return task_cls(data.payload)
...
class _RequestDataAdapter(_AbstractAdapter):
    ...
    def __init__(self, data):
        ...
        try:
            self._extract_action(data['action'])
...
class _ResponseDataAdapter(_AbstractAdapter):
    ...
    def __init__(self, data):
        ...
        self._extract_fields({'url': 'url'}.items())


class _TaskMapping(object):
    def __init__(self):
        self.tasks = []
        ...
        for task in self.tasks:
            ...
            self.task_to_name[task] = task.name
