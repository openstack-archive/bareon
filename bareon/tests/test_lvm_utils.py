...
class TestLvmUtils(unittest2.TestCase):
    ...
    def test_lvcreate_ok(self, mock_exec, mock_lvdisplay, mock_vgdisplay):
        ...
        expected_calls = [
            mock.call('lvcreate', '-y', '--wipesignatures', 'y', '--zero',
                      'y', '-L', '1000m', '-n', 'lvname', 'vgname',
                      check_exit_code=[0])
        ]
    ...
    def test_lvcreate_lv_name_collision(self, mock_exec, mock_lvdisplay,
        ...
        mock_vgdisplay.return_value = [{'name': 'testvg', 'free': 2000},
                                       {'name': 'some', 'free': 2000}]
        mock_lvdisplay.return_value = [{'name': 'testlv', 'vg': 'some'}]
        mock_exec.side_effect = [('', '')]
        expected_calls = [
            mock.call('lvcreate', '-y', '--wipesignatures', 'y', '--zero',
                      'y', '-L', '1000m', '-n', 'testlv', 'testvg',
                      check_exit_code=[0])
        ]
        lu.lvcreate('testvg', 'testlv', 1000)
        self.assertEqual(mock_exec.call_args_list, expected_calls)
